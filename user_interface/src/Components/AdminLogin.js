...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    // API call to login the admin user
    ...
    if (res && res.status === 200) {
      ...
      if(user.Role_id !== 2 || (user.Role_id === 2 && user.Status_id !== 1)) {
        setIsError(true);
      } else {
        dispatch(login(user))
        setUser(user)
      ...
      setIsError(true);
      console.log('Incorrent Credentials!')
    }
  }
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" minHeight="100vh">
      <Button variant="outlined" component={Link} to={`/admin`} sx={{ marginBottom: '24px' }}>
        <HomeIcon />
      </Button>
      <Typography variant="h4" component="h1">Admin Login</Typography>
      ...
      {isError && (
        <Typography variant="body2" color="error" mt={2}>
          Incorrect credentials or user doesn't exist, please try again!
        </Typography>
      )}
    </Box>
  );
